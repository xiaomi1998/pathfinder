...
export const getLocationOptions = () => {
  return Object.entries(LOCATION_MAPPING).map(([value, label]) => ({
    value,
    label
  }))
}
...
// 验证中文地区名称是否有效
export const isValidLocationName = (name: string): name is LocationName => {
  return name in LOCATION_NAME_TO_CODE
}
